...
# PDF Faylının adı (Fayl bu skriptlə eyni qovluqda olmalıdır)
PDF_FILENAME = 'mmsillabussu.pdf'

def extract_text_from_pdf(filename):
    """PDF-dən bütün mətni çıxarır."""
    text = ""
    try:
...
def parse_quiz_content(text):
    ...
    for line in lines:
        ...
        # Səhifə başlıqlarını və lazımsız sətirləri təmizləyirik
...
# Flask-ın render etməsi üçün vahid HTML şablonu
HTML_TEMPLATE = """
<!DOCTYPE html>
<html lang="az">
<head>
...
<body class="flex flex-col h-screen overflow-hidden">
    ...
    <header class="bg-blue-900 text-white shadow-md z-10">
        <div class="container mx-auto px-4 py-3 flex justify-between items-center">
            ...
            <div class="flex items-center space-x-4">
                ...
                <button onclick="finishQuiz()" class="bg-red-600 hover:bg-red-700 text-white px-4 py-1.5 rounded text-sm font-bold transition shadow">
                    Bitir
                </button>
            </div>
        </div>
        ...
        <main class="flex-1 overflow-y-auto p-4 md:p-8 relative" id="mainContainer">
            ...
            <div id="quizContent" class="max-w-4xl mx-auto hidden">
                ...
                <div class="flex justify-between items-center">
                    <button id="prevBtn" onclick="changeQuestion(-1)" class="px-6 py-3 bg-white border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 shadow-sm transition disabled:opacity-50">
                        <i class="fas fa-chevron-left mr-2"></i> Əvvəlki
                    </button>
                    <button id="toggleMapMobile" onclick="toggleMobileMap()" class="md:hidden px-4 py-3 bg-blue-100 text-blue-800 rounded-lg">
                        <i class="fas fa-th-large"></i>
                    </button>
                    <button id="nextBtn" onclick="changeQuestion(1)" class="px-6 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 shadow-md transition">
                        Növbəti <i class="fas fa-chevron-right ml-2"></i>
                    </button>
                </div>
            </div>
            ...
            <div id="resultsScreen" class="hidden max-w-4xl mx-auto">
                <div class="bg-white rounded-xl shadow-lg p-8 mb-8 text-center border-t-4 border-blue-600">
                    ...
                    <div class="grid grid-cols-3 gap-4 max-w-lg mx-auto mb-8">
                        <div class="p-3 bg-green-50 rounded border border-green-200">
                            <div class="text-xl font-bold text-green-600" id="correctCount">0</div>
                            <div class="text-xs text-green-800 uppercase font-semibold">Düzgün</div>
                        </div>
                        <div class="p-3 bg-red-50 rounded border border-red-200">
                            <div class="text-xl font-bold text-red-600" id="wrongCount">0</div>
                            <div class="text-xs text-red-800 uppercase font-semibold">Yanlış</div>
                        </div>
                        <div class="p-3 bg-gray-50 rounded border border-gray-200">
                            <div class="text-xl font-bold text-gray-600" id="emptyCount">0</div>
                            <div class="text-xs text-gray-800 uppercase font-semibold">Boş</div>
                        </div>
                    </div>
                ...
                <div class="bg-white rounded-xl shadow-lg p-8">
                    <h3 class="text-xl font-bold text-gray-800 mb-6">Ətraflı Analiz</h3>
    ...
    <div id="mobileMapModal" class="fixed inset-0 bg-black bg-opacity-50 z-40 hidden md:hidden" onclick="toggleMobileMap()">
        <div class="absolute bottom-0 left-0 right-0 bg-white rounded-t-xl p-4 max-h-[70vh] overflow-y-auto" onclick="event.stopPropagation()">
            <div class="grid grid-cols-5 gap-3" id="mobileQuestionMap"></div>
        </div>
    </div>
    ...
    <script>
        ...
        function renderQuestion(index) {
            ...
            q.options.forEach((opt, idx) => {
                ...
                div.innerHTML = `
                    <input type="radio" name="q_${index}" id="opt_${idx}" class="option-input hidden peer" 
                        ${isChecked ? 'checked' : ''} onchange="selectOption(${index}, ${idx})">
                    <label for="opt_${idx}" class="option-label block w-full p-4 bg-gray-50 border-2 border-transparent rounded-lg hover:bg-gray-100 peer-checked:bg-blue-50 peer-checked:border-blue-500 peer-checked:text-blue-700 cursor-pointer transition-all">
                        <div class="flex items-center">
                            <div class="w-6 h-6 border-2 border-gray-300 rounded-full flex items-center justify-center mr-3 peer-checked:bg-blue-500 peer-checked:border-blue-500">
                                <div class="w-2.5 h-2.5 bg-white rounded-full ${isChecked ? '' : 'hidden'}"></div>
                            </div>
                            <span>${opt.text}</span>
                        </div>
                    </label>
        ...
        function renderMap() {
            const mapHtml = questions.map((_, i) => 
                `<button id="mapBtn_${i}" onclick="renderQuestion(${i})" class="map-btn w-full aspect-square flex items-center justify-center border rounded text-sm font-medium hover:bg-gray-100 transition">${i + 1}</button>`
            ).join('');
            
            document.getElementById('questionMap').innerHTML = mapHtml;
            document.getElementById('mobileQuestionMap').innerHTML = mapHtml;
        }

        function updateMapHighlights() {
            questions.forEach((_, i) => {
                const btn = document.getElementById(`mapBtn_${i}`);
                const mobBtn = document.getElementById(`mobileQuestionMap`).children[i];
                
                let cls = "map-btn w-full aspect-square flex items-center justify-center border rounded text-sm font-medium transition ";
                
                if (i === currentQuestionIndex) {
                    cls += "border-yellow-500 ring-2 ring-yellow-200 z-10 ";
                } else {
                    cls += "border-gray-200 ";
                }
                ...
                if (userAnswers[i] !== null) {
                    ...
                    cls += "bg-white text-gray-700 hover:bg-gray-50";
                }
                
                btn.className = cls;
                if(mobBtn) mobBtn.className = cls;
            });
        }
        ...
        function finishQuiz() {
            ...
            questions.forEach((q, i) => {
                ...
                let status = 'empty';
                if(userAnsIdx !== null) {
                    if(q.options[userAnsIdx].isCorrect) {
                        correct++;
                        status = 'correct';
                    } else {
                        wrong++;
                ...
                card.innerHTML = `
                    <div class="flex gap-3">
                        <div class="font-bold text-gray-500">${i+1}.</div>
                        <div class="flex-1">
                            <p class="font-medium mb-2">${q.text}</p>
                            <div class="text-sm space-y-1">
                                ${status !== 'empty' ? 
                                    `<div class="${status === 'correct' ? 'text-green-700 font-bold' : 'text-red-700 font-bold'}">Sizin cavab: ${q.options[userAnsIdx].text}</div>` : 
                                    `<div class="text-gray-500 italic">Cavab verilməyib</div>`}
                                <div class="text-green-700 bg-green-100 inline-block px-2 py-1 rounded text-xs font-bold mt-1">
                                    Doğru: ${correctOpt ? correctOpt.text : 'Təyin olunmayıb'}
                                </div>
                            </div>
                        </div>
                        <div class="text-xl">
                            ${status === 'correct' ? '<i class="fas fa-check text-green-500"></i>' : 
                              status === 'wrong' ? '<i class="fas fa-times text-red-500"></i>' : 
        ...
        function toggleMobileMap() {
            const el = document.getElementById('mobileMapModal');
...
def get_questions():
    if not os.path.exists(PDF_FILENAME):
        return jsonify({'error': f"'{PDF_FILENAME}' faylı tapılmadı. Zəhmət olmasa faylı proqramın olduğu qovluğa əlavə edin."})
    
    text = extract_text_from_pdf(PDF_FILENAME)
    if not text:
        return jsonify({'error': "PDF faylı oxuna bilmədi. Faylın zədələnmədiyindən əmin olun."})

    all_questions = parse_quiz_content(text)
    
    if not all_questions:
        return jsonify({'error': "Suallar tapılmadı. PDF formatının düzgün olduğundan əmin olun."})

    # Suallardan təsadüfi 50 dənəsini seç (əgər azdırsa, hamısını götür)
    count = min(len(all_questions), 50)
    selected_questions = random.sample(all_questions, count)
    
    # Hər sualın variantlarını da qarışdır
    for q in selected_questions:
        q['original_id'] = q['id'] # Orijinal nömrəni saxla
        random.shuffle(q['options'])
        
    return jsonify(selected_questions)

if __name__ == '__main__':
    print("Tətbiq işə düşdü! http://127.0.0.1:5000 ünvanına daxil olun.")
    app.run(debug=True)